...
export class ClientsService {
  ...
  async getClients(organizationId?: string) {
    ...
    return clients.map((client) => ({
      ...
      totalAllocatedMinor: client.jobs.flatMap((job) => job.allocations).reduce((sum, allocation) => sum + allocation.allocatedMinor, 0n),
      totalLockedMinor: lots.filter((lot) => lot.ownerId === client.id && lot.status === 'LOCKED').reduce((sum, lot) => sum + lot.currentAmountMinor, 0n),
      jobsCount: client.jobs.length,
      allocationsCount: client.jobs.reduce((sum, job) => sum + job.allocations.length, 0)
    }));
  }
