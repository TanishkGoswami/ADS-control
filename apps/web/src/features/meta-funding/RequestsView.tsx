...
export const RequestsView: React.FC = () => {
  ...
  return <div className="funding-panel">
    ...
    {open&&<div className="funding-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><form className="funding-dialog" onSubmit={e=>{e.preventDefault();create.mutate()}}><div className="p-4 border-b border-[#e4e7eb] flex justify-between"><div><div className="font-semibold">Create funding request</div><div className="funding-muted">Amounts are reserved only after mapping.</div></div><button type="button" onClick={()=>setOpen(false)} aria-label="Close"><X className="w-4 h-4"/></button></div><div className="p-4 grid gap-3">
      <label className="text-xs font-semibold">Fund lot<select required className="funding-input w-full mt-1" value={form.fundLotId} onChange={e=>setForm({...form,fundLotId:e.target.value})}><option value="">Select available source</option>{eligible.map((l:any)=><option key={l.id} value={l.id}>{l.lotCode} · {money(l.availableAmountMinor)} available</option>)}</select></label>
      ...
      <label className="text-xs font-semibold">Target Ad Account (optional)<select className="funding-input w-full mt-1" value={form.targetAdAccountId} onChange={e=>setForm({...form,targetAdAccountId:e.target.value})}><option value="">Any eligible account</option>{(accounts.data||[]).map((a:any)=><option key={a.id} value={a.id}>{a.name} · {a.metaAdAccountId}</option>)}</select></label>
